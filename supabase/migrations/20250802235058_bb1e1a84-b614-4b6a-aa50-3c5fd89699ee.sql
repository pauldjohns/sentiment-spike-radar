-- Create missing stocktwits_api_usage table for rate limiting
CREATE TABLE public.stocktwits_api_usage (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  api_key TEXT NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Enable RLS on stocktwits_api_usage
ALTER TABLE public.stocktwits_api_usage ENABLE ROW LEVEL SECURITY;

-- Create RLS policy for stocktwits_api_usage (service role only)
CREATE POLICY "Service role can manage stocktwits api usage" 
ON public.stocktwits_api_usage 
FOR ALL 
USING (auth.role() = 'service_role');

-- Create missing sentiment_model_fallback_log table
CREATE TABLE public.sentiment_model_fallback_log (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  ticker TEXT NOT NULL,
  message_id TEXT NOT NULL,
  raw_message TEXT NOT NULL,
  fallback_sentiment TEXT NOT NULL,
  reason TEXT NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Enable RLS on sentiment_model_fallback_log
ALTER TABLE public.sentiment_model_fallback_log ENABLE ROW LEVEL SECURITY;

-- Create RLS policy for sentiment_model_fallback_log (service role only)
CREATE POLICY "Service role can manage sentiment model fallback log" 
ON public.sentiment_model_fallback_log 
FOR ALL 
USING (auth.role() = 'service_role');

-- Fix database function security vulnerabilities by adding proper search_path
