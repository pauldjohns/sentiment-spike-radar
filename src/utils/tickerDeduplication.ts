 ...
 */
export function startOfTodayEasternISO(referenceDate: Date = new Date()): string {
...
export class TickerDeduplicationService {
  ...
  static async filterUniqueTickersForWindow(
    ...
    const selectedTodaySet = new Set(
      alreadySelectedToday?.map(signal => signal.ticker) || []
    );
  ...
  static async filterUniqueSignalsForWindow<T extends { ticker: string }>(
    ...
    const deduplicationResult = await this.filterUniqueTickersForWindow(candidateTickers, timeWindow);
